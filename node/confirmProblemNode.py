...
def confirmProblemNode(confirmState:ConfirmProblemState) -> dict:
    ...
    math_only_section = ""
    if spec.subject_str == "수학":
        math_only_section = """        - 수의 범위: {number_range}
            - 연산 단계수: {operation_steps}
            - 연산 종류: {operation_types}
    """.format(
            number_range=spec.number_range,
            operation_steps=spec.operation_steps,
            operation_types=", ".join(spec.operation_types) if spec.operation_types else "없음",
        )

    human_prompt = """다음은 문항 설계 명세와 실제로 생성된 문제입니다. 위 검증 기준에 따라 비교 검증해주세요.
            해당 과목({subject})에 적용되지 않는 항목은 무시하고, 해당 과목 전용 검증 기준을 우선 적용하세요.
    ...
    {math_only_section}        - 문장제 여부: {is_word_problem}
            ...
            """.format(
                ...
                math_only_section=math_only_section,
    ...
    if not result.is_confirm:
        return {
                ...
                "confirm_feedback": result.confirm_feedback
        }
    else:
        return {
                "check_problemState": result,
